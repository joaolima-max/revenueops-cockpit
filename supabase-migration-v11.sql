...
INSERT INTO "PipelineFunil" ("id","nome","descricao","area","ordem","ativo","exigeCliente") VALUES
  ('fnl_vendas',     'Vendas',     'Da prospecção ao fechamento do negócio.',        'Comercial',   1, true, false),
  ('fnl_onboarding', 'Onboarding', 'Implantação do cliente até entrar em produção.', 'Operacional', 2, true, true),
  ('fnl_operacoes',  'Operações',  'Cliente em produção: ativação e sustentação.',   'Operacional', 3, true, true)
ON CONFLICT ("id") DO NOTHING;

-- ------------------------------------------------------------ 7. As etapas

INSERT INTO "PipelineEtapa" ("id","funilId","nome","ordem","tipo") VALUES
  ('etp_vnd_prospeccao',   'fnl_vendas', 'Prospecção',   1, 'NORMAL'),
  ('etp_vnd_qualificacao', 'fnl_vendas', 'Qualificação', 2, 'NORMAL'),
  ('etp_vnd_proposta',     'fnl_vendas', 'Proposta',     3, 'NORMAL'),
  ('etp_vnd_negociacao',   'fnl_vendas', 'Negociação',   4, 'NORMAL'),
  ('etp_vnd_fechamento',   'fnl_vendas', 'Fechamento',   5, 'NORMAL'),
  ('etp_vnd_ganho',        'fnl_vendas', 'Ganho',        6, 'GANHO'),
  ('etp_vnd_perdido',      'fnl_vendas', 'Perdido',      7, 'PERDIDO'),

  ('etp_onb_kickoff',      'fnl_onboarding', 'Kickoff',         1, 'NORMAL'),
  ('etp_onb_contrato',     'fnl_onboarding', 'Contrato',        2, 'NORMAL'),
  ('etp_onb_taxas',        'fnl_onboarding', 'Ajuste de Taxas', 3, 'NORMAL'),
  ('etp_onb_integracao',   'fnl_onboarding', 'Integração',      4, 'NORMAL'),
  ('etp_onb_homologacao',  'fnl_onboarding', 'Homologação',     5, 'NORMAL'),
  ('etp_onb_golive',       'fnl_onboarding', 'Go Live',         6, 'NORMAL'),

  ('etp_ope_ativacao',     'fnl_operacoes', 'Ativação',      1, 'NORMAL'),
  ('etp_ope_monitoramento','fnl_operacoes', 'Monitoramento', 2, 'NORMAL'),
  ('etp_ope_sustentacao',  'fnl_operacoes', 'Sustentação',   3, 'NORMAL')
ON CONFLICT ("id") DO NOTHING;
...
INSERT INTO "PipelinePermissao"
  ("id","funilId","role","ver","editar","mover","criar","transferir","administrar","apenasProprios") VALUES
  ('prm_vnd_comercial',  'fnl_vendas',     'COMERCIAL',   true, true, true, true, true, false, true),
  ('prm_onb_operacional','fnl_onboarding', 'OPERACIONAL', true, true, true, true, true, false, false),
  ('prm_ope_operacional','fnl_operacoes',  'OPERACIONAL', true, true, true, true, true, false, false)
ON CONFLICT ("id") DO NOTHING;
