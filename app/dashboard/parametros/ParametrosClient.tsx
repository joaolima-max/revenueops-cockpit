...
export default function ParametrosClient({ parametros: initial, isAdmin }: { parametros: Parametro[]; isAdmin: boolean }) {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="t-h1 text-fg">Parâmetros do Sistema</h1>
        <p className="text-subtle text-sm mt-0.5">Critérios de risco, thresholds de alertas e parâmetros operacionais</p>
      </div>
      ...
      {grupos.map(grupo => (
        <div key={grupo} className="bg-surface border border-line rounded-xl overflow-hidden">
          <div className="px-5 py-3 border-b border-line flex items-center gap-2">
            <h3 className="t-h3 text-fg">{GRUPO_LABELS[grupo] || grupo}</h3>
          </div>
          <div className="overflow-x-auto"><table className="w-full min-w-[40rem]">
            <thead>
              <tr className="border-b border-line">
                {['Parâmetro', 'Descrição', 'Valor', 'Ação'].map(h => (
                  <th key={h} className="t-label text-subtle px-5 py-2.5 text-left">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {parametros.filter(p => p.grupo === grupo).map(p => {
                ...
                return (
                  <tr key={p.id} className="border-b border-line hover:bg-[var(--bp-hover)]">
                    <td className="px-5 py-3">
                      <p className="text-fg text-sm font-medium">{p.label}</p>
                      <p className="text-subtle text-xs font-mono">{p.chave}</p>
                    </td>
                    <td className="px-5 py-3 text-subtle text-xs max-w-xs">{p.descricao || '—'}</td>
                    <td className="px-5 py-3">
                      {isEdit ? (
                        <input type={p.tipo === 'TEXT' ? 'text' : 'number'} step="0.1"
                          ...
                          autoFocus
                          className="bp-field w-24 text-sm" />
                      ) : (
                        <span className="text-pos font-semibold text-sm">{fmtVal(p)}</span>
                      )}
                    </td>
                    <td className="px-5 py-3">
                      {isAdmin && (isEdit ? (
                        <div className="flex gap-2">
                          <button onClick={() => save(p)} disabled={saving === p.id}
                            className="text-xs px-2 py-1 bg-pos/10 text-pos rounded hover:bg-pos/20 disabled:opacity-50">
                            {saving === p.id ? '...' : 'Salvar'}
                          </button>
                          <button onClick={() => cancel(p.id)}
                            className="text-xs px-2 py-1 bg-surface-2 text-muted rounded hover:bg-surface-2">
                            Cancelar
                          </button>
                        </div>
                      ) : (
