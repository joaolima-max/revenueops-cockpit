  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
        {REPORTS.map(r => (
          <div key={r.id} className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden flex flex-col hover:border-gray-700 transition-colors">
            <div className="h-1" style={{ background: r.accent }} />
            <div className="p-5 flex flex-col flex-1">
              <div className="flex items-start justify-between gap-3 mb-2">
                <h2 className="text-sm font-semibold text-white leading-tight">{r.title}</h2>
                {r.confidential && (
                  <span className="text-[9px] px-1.5 py-0.5 rounded bg-red-500/10 text-red-400 border border-red-500/20 flex-shrink-0 font-semibold tracking-wide">CONF.</span>
                )}
              </div>
              <p className="text-[11px] text-gray-500 mb-4 leading-relaxed">{r.description}</p>
              <div className="flex flex-wrap gap-1.5 mb-5">
                {r.metrics.map(m => (
                  <span key={m} className="text-[10px] px-2 py-0.5 rounded-full bg-gray-800 text-gray-400 border border-gray-700/50">{m}</span>
                ))}
              </div>
      ...
      <div className="mt-6 pt-6 border-t border-gray-800">
        <div className="flex items-center justify-between">
          <p className="text-xs text-gray-700">Analytics consolidado com gráficos interativos</p>
          <Link href="/dashboard/relatorios/analitico" className="text-xs text-gray-600 hover:text-gray-400 transition-colors">
            Relatório Analítico →
          </Link>
        </div>
      </div>
